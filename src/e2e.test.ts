...
const withTestGateway = async (
	...
	try {
		...
		while (loc && !loc.startsWith('http://localhost:9999')) { r = await fetch(loc, {redirect: 'manual'}); loc = r.headers.get('location') ?? ''; } // eslint-disable-line no-await-in-loop
		const code = new URL(loc).searchParams.get('code')!;
		const tokRes = await fetch(`${gwUrl}/token`, {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: new URLSearchParams({grant_type: 'authorization_code', code, client_id: clientId, code_verifier: codeVerifier, redirect_uri: 'http://localhost:9999/callback'}).toString()});
		...
		const gwMcpCall = async (method: string, params: Record<string, unknown> = {}, id = 1) =>
			fetch(`${gwUrl}/mcp`, {method: 'POST', headers: {'Content-Type': 'application/json', Accept: 'application/json, text/event-stream', Authorization: `Bearer ${gwToken}`}, body: JSON.stringify({jsonrpc: '2.0', method, id, params})});
